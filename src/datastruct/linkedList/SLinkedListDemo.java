 ...
 */
class SLinkedList {
    public Node head;  //如果不定义头节点那怎么办？一般都要定义
...
//    这里面的遍历链表的方法（20行）中，参数node表示从node节点开始遍历，不一定要从head节点遍历。
    ...
    public void remove(int index) {
        ...
        if (index == 1) {
            ...
            current.next = current.next.next;  //保证current.next不为空
            size--;
        }
    ...
    //方法3，修改
    //思路：找到要修改的节点的前节点，然后将加入的新节点的下一节点指向被修改节点的下一个节点，最后将被修改的节点的前节点的下一节点指向新节点
    //修改和删除都需要找到变化目标节点的前节点进行操作
    public void update(int index, Node node) {
        ...
        Node current = head;
        if (index == 1) {
            node.next = head.next; //结论1，凡是设计新节点，都要先为新节点的指针赋值。（从右到左原则）
            head = node;
        } else {
            for (int i = 0; i < index - 2; i++) { //注意是减2
                current = current.next;
    ...
    public Node findLastNode(Node node, int index) {
        ...
        //先让second和first之间相隔k-1个位置，这样当second都末尾时，first就在末尾前的第k个地方
        //注意是相隔k-1，不占位相隔和站位相隔：站位相隔k，等于不占位相隔k-1。
        ...
        while (second.next != null) {
            first = first.next;
            second = second.next; //更新完之后while条件才判断（即更新后判断），而if可以前后判断。也即if second.next == null， break
        }
    ...
    //添加到本数据对象中，因此不需要new一个新的进来。此处入口也可以做成传入List
    public void mergeLinkedList(SLinkedList sLinkedList1, SLinkedList sLinkedList2) {
    ...
    //方法7，单链表的反转。在反转指针前一定要保存下个结点的指针。【出现频率最高】
...
//            current.next = reverseHead.next;
    ...
    public boolean hasCycle() {
        ...
        //循环到链尾second != null，此时second.next = null，也即循环体中的second一定不为null；second.next != null,针对最后循环时当前节点在末尾上
...
//        while (second != null){
    ...
    //方法9，取出有环链表中，环的长度。思路：需要先利用上面的第7小节中的hasCycle方法（判断链表是否有环的那个方法），这个方法的返回值是boolean型，
    //但是现在要把这个方法稍做修改，让其返回值为相遇的那个结点。然后，我们拿到这个相遇的结点就好办了，这个结点肯定是在环里嘛，
    // 我们可以让这个结点对应的指针一直往下走，直到它回到原点，就可以算出环的长度了。
    ...
    public int getCycleLength(Node nodeInCircle) {
        if (head == null) {
            return 0;
        }

        Node current = nodeInCircle;
        int cycleLength = 0;
        while (true) {  //默认有环
            current = current.next;
            cycleLength++;
            if (current == nodeInCircle) {
                return cycleLength;
            }
        }
    }

    //方法10，单链表中，取出环的起始点.思路：这里我们需要利用到上面第8小节的取出环的长度的方法getCycleLength，用这个方法来获取环的长度length。
    // 拿到环的长度length之后，需要用到两个指针变量first和second，先让second指针走length步；然后让first指针和second指针同时各走一步，
    ...
    public void sortByData() {
        ...
        while (cur.next != null) {
            //找到需要交换的节点的值，每次都要遍历所有cur之后的所有数，属于插入排序？
            next = cur.next;
            while (next != null) {
    ...
    public void distinctLink() {
        ...
        while (current != null) {
            if (map.containsKey(current.data)) {//如果hashtable中已存在该结点，则跳过该结点
                pre.next = current.next;  //跳过该结点
